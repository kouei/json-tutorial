...
static void test_parse_array() {
    ...
    for(i = 0; i < 4; ++i) {
        ...
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(lept_get_array_element(&v, i)));
        for(j = 0; j < i; ++j) {
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(lept_get_array_element(&v, i), j)));
            EXPECT_EQ_DOUBLE((double) j, lept_get_number(lept_get_array_element(lept_get_array_element(&v, i), j)));
        }
    }
    lept_free(&v);
}
