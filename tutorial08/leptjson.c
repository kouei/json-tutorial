    ...
    switch (src->type) {
        ...
        case LEPT_ARRAY: {
            ...
            for(i = 0; i < src->u.a.size; ++i) {
                lept_value * v = lept_pushback_array_element(dst);
        ...
        case LEPT_OBJECT:
            ...
            for(i = 0; i < src->u.o.size; ++i) {
                lept_value * v = lept_set_object_value(dst, src->u.o.m[i].k, src->u.o.m[i].klen);
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT: {
            ...
            for(i = 0; i < lhs->u.o.size; ++i) {
                lept_value * rhs_value = lept_find_object_value((lept_value *)rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen);
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    ...
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    /* \todo */
    to_set = lept_find_object_value(v, key, klen);
    if(to_set == NULL) {
        lept_member* m = lept_pushback_object_member(v);
...
void lept_remove_object_value(lept_value* v, size_t index) {
    ...
    /* \todo */
    for(i = index; i + 1 < v->u.o.size; ++i) {
        lept_member * lhs = &v->u.o.m[i];
        lept_member * rhs = &v->u.o.m[i + 1];
        lept_member temp;
...
void lept_popback_object_member(lept_value* v) {
    ...
    --v->u.o.size;
    free(v->u.o.m[v->u.o.size].k);
    lept_free(&v->u.o.m[v->u.o.size].v);
}
